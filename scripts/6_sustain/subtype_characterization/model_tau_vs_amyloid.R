...
pacs <- colnames(master)[str_detect(colnames(master), 'PAC.*WScore')]
...
for (i in 1:length(ptcs)) {
  ...
  p <- ggplot(data=training, aes(x = SummarySUVRAmyloid, y = !!sym(ptc), color = Subtype)) +
...
ggarrange(plotlist = plots, common.legend = TRUE, legend="top", labels = NULL)
